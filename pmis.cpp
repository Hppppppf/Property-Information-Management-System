...
void PMIS::on_tableWidgetFeedbackInfo_cellClicked(int row, int column)
{
    ...
    Document doc("Data.xlsx");
    doc.selectSheet("报修");
    ui.labelFeedbackType->setText("编号："+doc.read(row+2,4).toString());
    ui.textBrowserFeedbackDetail->setText(doc.read(row+2,5).toString());
    if (doc.read(row+2,3) == "处理中") ui.pushButtonDone->setEnabled(true);
    else if (doc.read(row+2,3) == "待处理") ui.pushButtonDoing->setEnabled(true);
}
//改变报修信息状态
...
void PMIS::on_tableWidgetFeedbackInfo2_cellClicked(int row, int column)
{
    ...
    doc.selectSheet("投诉");
    ui.labelFeedbackType->setText("编号："+doc.read(row+2,4).toString());
    ui.textBrowserFeedbackDetail2->setText(doc.read(row+2,5).toString());
    if (doc.read(row+2,3) == "处理中") ui.pushButtonDone2->setEnabled(true);
    else if (doc.read(row+2,3) == "待处理") ui.pushButtonDoing2->setEnabled(true);
}
//改变投诉信息状态
